...
**Durable pending-publication protocol (rev. H):** pending-manifest publication uses a second private intent, not a receipt entry or ownership claim. With the receipt lock held first and the manifest lock held second, Styrn fsyncs the prepared intent before replacing the manifest. The intent binds the exact receipt prefix and pending-publication prefix, receipt scope/principal/path, manifest scope/principal/path and machine ID, canonical before/after manifest SHA-256 digests, the complete canonical candidate, and the target receipt links. Its prepared target is the effective current pending epoch, so an immediately recurring action cannot be suppressed while receipt finalization is incomplete. Recovery accepts only two manifest states: an exact `before` digest retries the stored candidate and then appends the checkpoint; an exact `after` digest only appends the checkpoint. Any other digest is `setup.receipt_conflict` and retains the intent and receipt evidence. Ordinary applied/pending entry appends may extend the receipt after the captured prefix but must preserve that prefix and its publication history exactly. The intent is removed with verified no-follow identity-safe deletion only after the checkpoint is durable; recovery never infers a candidate from an unbound path or a newly projected plan.
