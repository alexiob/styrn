...
- [ ] **T0.8** — Capability probe layer, shared with doctor (15.2, 6.5)
  ...
  - [ ] − negative: an observation failure such as permission denial, I/O failure, unsafe/replaced evidence, or malformed/contradictory output yields `Unknowable` rather than `false` — the plan must never silently treat "could not determine" as "not installed". `Unsupported` represents an absent expected package backend, including apt with either `apt-get` or `dpkg-query` absent, and only the dependent action becomes `NeedsHuman`; normal absence never becomes `setup.probe_failed`. Reject WSL, invalid UTF-8/NUL/size/token/quote/escape input, duplicate keys, contradictory cross-family `ID_LIKE`, fake `PATH` managers, unsafe manager evidence, manager query contradictions, and executable drift without erasing unrelated capabilities or writing state.
