...
pub(super) fn create_worker_directory_layout(
    ...
    let children = match staged_children {
        ...
        None => open_or_create_worker_children(&root_parent, &directory, expected_uid, false)?,
    };
...
fn open_or_create_worker_directory_at(
    ...
    .map_err(|error| {
        io::Error::new(
            error.kind(),
            format!("publishing worker directory failed: {error}"),
        )
    })
}
...
fn create_or_open_complete_worker_root(
    ...
    let children = match open_or_create_worker_children(
        root_parent,
        &staged.directory,
        expected_uid,
        !staged.created,
...
fn publish_staged_worker_directory(
    ...
    } == 0
    {
        ...
        if existing_must_be_canonical && !complete_root {
            ...
            staged.directory.sync_all().map_err(|error| {
                io::Error::new(
                    error.kind(),
